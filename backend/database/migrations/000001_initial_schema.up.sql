...
CREATE TABLE users (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), display_name VARCHAR(150), email CITEXT UNIQUE,
...
CREATE TABLE user_identities (
 ...
 provider VARCHAR(20) NOT NULL CHECK (provider IN ('LINE')), provider_user_id VARCHAR(255) NOT NULL,
 created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE email_verification_tokens (
 ...
 otp_hash TEXT NOT NULL, expires_at TIMESTAMPTZ NOT NULL, used_at TIMESTAMPTZ,
...
CREATE TABLE refresh_tokens (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 family_id UUID NOT NULL DEFAULT gen_random_uuid(), token_hash TEXT NOT NULL UNIQUE,
 replaced_by_token_id UUID REFERENCES refresh_tokens(id) ON DELETE SET NULL,
 expires_at TIMESTAMPTZ NOT NULL, last_used_at TIMESTAMPTZ, revoked_at TIMESTAMPTZ,
 created_ip INET, user_agent TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
 CHECK (expires_at>created_at), CHECK (revoked_at IS NULL OR revoked_at>=created_at)
);
CREATE TABLE stocks (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), symbol VARCHAR(20) NOT NULL,
 provider VARCHAR(30) NOT NULL DEFAULT 'FINNHUB', provider_symbol VARCHAR(100),
 name VARCHAR(255), exchange VARCHAR(50), currency VARCHAR(10),
 status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE','DISABLED')),
 disabled_reason TEXT, disabled_at TIMESTAMPTZ, last_quote_price NUMERIC(20,8), last_quote_at TIMESTAMPTZ,
 created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
 UNIQUE(provider,symbol), CHECK (symbol=UPPER(symbol)), CHECK (last_quote_price IS NULL OR last_quote_price>=0),
 CHECK ((status='ACTIVE' AND disabled_at IS NULL) OR status='DISABLED')
);
CREATE TABLE watchlists (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
 name VARCHAR(100) NOT NULL DEFAULT 'My Watchlist', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
 updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE UNIQUE INDEX uq_watchlists_user_name ON watchlists(user_id,LOWER(name));
CREATE TABLE watchlist_stocks (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), watchlist_id UUID NOT NULL REFERENCES watchlists(id) ON DELETE CASCADE,
 stock_id UUID NOT NULL REFERENCES stocks(id) ON DELETE RESTRICT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE alerts (
 ...
 triggered_at TIMESTAMPTZ, disabled_reason TEXT, disabled_at TIMESTAMPTZ,
 created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE UNIQUE INDEX uq_alerts_active_definition ON alerts(user_id,stock_id,condition,target_price) WHERE status='ACTIVE';
CREATE TABLE alert_logs (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), alert_id UUID REFERENCES alerts(id) ON DELETE SET NULL,
 user_id UUID REFERENCES users(id) ON DELETE SET NULL, channel VARCHAR(20) NOT NULL CHECK (channel IN ('LINE','EMAIL')),
 status VARCHAR(20) NOT NULL CHECK (status IN ('PENDING','SENT','FAILED')), message TEXT,
 triggered_price NUMERIC(20,8) NOT NULL CHECK (triggered_price>=0), provider_message_id VARCHAR(255),
 attempted_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), sent_at TIMESTAMPTZ, error_message TEXT,
...
CREATE TABLE line_webhook_events (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), line_event_id VARCHAR(255) NOT NULL UNIQUE,
 line_user_id VARCHAR(255), event_type VARCHAR(50) NOT NULL, payload JSONB NOT NULL,
 status VARCHAR(20) NOT NULL DEFAULT 'RECEIVED' CHECK (status IN ('RECEIVED','PROCESSED','FAILED')),
 processed_at TIMESTAMPTZ, error_message TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
 CHECK ((status='PROCESSED' AND processed_at IS NOT NULL) OR status<>'PROCESSED'),
 CHECK ((status='FAILED' AND error_message IS NOT NULL) OR status<>'FAILED')
);
CREATE TABLE application_logs (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), level VARCHAR(20) NOT NULL CHECK (level IN ('DEBUG','INFO','WARN','ERROR')),
 service VARCHAR(100) NOT NULL, code VARCHAR(100), message TEXT NOT NULL, details JSONB,
 request_id VARCHAR(100), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE TABLE admin_audit_logs (
 id UUID PRIMARY KEY DEFAULT gen_random_uuid(), admin_user_id UUID REFERENCES users(id) ON DELETE SET NULL,
 action VARCHAR(100) NOT NULL, target_type VARCHAR(50) NOT NULL, target_id TEXT,
 ip_address INET, user_agent TEXT, details JSONB, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
