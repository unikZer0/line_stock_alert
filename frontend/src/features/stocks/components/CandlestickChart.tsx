...
export function CandlestickChart({ candles }: { candles: StockCandle[] }) {
  ...
  const intraday = new Date(candles[candles.length - 1].timestamp).getTime() - new Date(candles[0].timestamp).getTime() < 2 * 86_400_000;
  const timelineIndexes = Array.from(new Set([0, Math.floor((candles.length - 1) / 3), Math.floor((candles.length - 1) * 2 / 3), candles.length - 1]));
  ...
  function selectCandle(event: PointerEvent<SVGSVGElement>) {
    ...
    const decimalIndex = Math.max(0, Math.min(candles.length - 1, (pointerX - left - step / 2) / step));
    const lowerIndex = Math.floor(decimalIndex);
    const upperIndex = Math.min(candles.length - 1, Math.ceil(decimalIndex));
    const fraction = decimalIndex - lowerIndex;
    const lowerTime = new Date(candles[lowerIndex].timestamp).getTime();
    const upperTime = new Date(candles[upperIndex].timestamp).getTime();
    setPointer({ x: pointerX, y: pointerY, index: Math.round(decimalIndex), time: lowerTime + (upperTime - lowerTime) * fraction });
  }

  const hovered = pointer === null ? null : candles[pointer.index];
  ...
  return <svg className="candle-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Interactive historical candlestick price chart"
    ...
    {timelineIndexes.map((index) => <text key={index} x={x(index)} y={height-12} textAnchor={index === 0 ? "start" : index === candles.length-1 ? "end" : "middle"} className="chart-axis-label">{formatTimeline(candles[index].timestamp, intraday)}</text>)}
